...
NUM_OBS_LIST = [100, 300]


def compute_results(img_arr, num_obs, cell_size, blob_size, alpha):

    # true coefs of theta
    coeffs_true = generate_coeff_vector(img_arr, num_obs, cell_size, blob_size)

    # V1 - SVD
    measurement_matrix_V1, V1_y = generate_V1_observation(img_arr, num_obs, cell_size, blob_size, None)
    ...
    # Pixel - SVD
    measurement_matrix_pix, pixel_y = generate_pixel_observation(img_arr, num_obs)
    ...
    # Gauss - SVD
    measurement_matrix_gauss, gaussian_y = generate_gaussian_observation(img_arr, num_obs)
...
def main():
    ...
    # index into results like so: results[100]["V1"]["a_est"]
    results = {
        num_obs: compute_results(small_img_arr_gray, num_obs, CELL_SIZE, BLOB_SIZE, ALPHA)
    ...
    pc_scatter_plots(results, 100, "figures/PC_scatter_100.svg")
    pc_scatter_plots(results, 300, "figures/PC_scatter_300.svg")

    compare_smoothed_errors(results, [100, 300], "figures/smoothed_error_log_100_300.svg")

    plot_first_pc(results, num_obs=100,
                  title="First Principal Component (30 x 30) (100 Obs)",
                  fileName="figures/pc_first_images_100.png")

    plot_first_pc(results, num_obs=300,
                  title="First Principal Component (30 x 30) (300 Obs)",
                  fileName="figures/pc_first_images_300.png")

    plot_top_pcs(results, num_obs=100, num_pcs=3,
                    title="Top 3 Principal Components per Method (30×30) (100 obs)",
                    fileName="figures/pc_top3_images_100_labeled.png"
    )
    plot_top_pcs(results, num_obs=300, num_pcs=3,
                    title="Top 3 Principal Components per Method (30×30) (300 obs)",
                    fileName="figures/pc_top3_images_300_labeled.png"
    )
