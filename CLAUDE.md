...
- **`src/sweep_figures.py`** — builds the paper figures for one solver from its consolidated sweep CSVs; `lasso_sweep_figures.py` and `bp_sweep_figures.py` (repo root) are thin drivers over it (`python lasso_sweep_figures.py`, `python bp_sweep_figures.py`). `varying_alpha` is offered only for penalised solvers. Output defaults to `figures/<algorithm>/paper_new/`.
